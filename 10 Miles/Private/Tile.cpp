...
void CTile::LateTick(_float fTimeDelta)
{
	...
	m_pRendererCom->Add_vTIle(ImgNum, this);
}
...
HRESULT CTile::SetUp_Components()
{
	...
	/* For.Com_VIBuffer */
	if (FAILED(__super::SetUp_Components(TEXT("Com_VIBuffer"), m_Current_Level, TEXT("Prototype_Component_VIBuffer_Tile"), (CComponent**)&m_pVIBufferCom)))
		return E_FAIL;
...
CGameObject * CTile::Clone(void * pArg)
{
	...
	{
		MSG_BOX(TEXT("Failed to Created asdCTile"));
		Safe_Release(pInstance);
	}
