...
CLevel_Event::CLevel_Event(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CLevel(pGraphic_Device)
{

}

HRESULT CLevel_Event::NativeConstruct()
{
	...
	if (FAILED(Ready_Layer_UI(TEXT("Layer_UI"))))
		return E_FAIL;
	Load_Data(L"../../Data/Event/Object.dat", LEVEL_EVENT);
	Load_Cube_Data(L"../../Data/Event/Cube.dat", LEVEL_EVENT);
	Load_Deco_Data(L"../../Data/Event/Deco.dat", LEVEL_EVENT);
	//Load_Tile_Data(L"../../Data/Event/Tile.dat", LEVEL_EVENT);
	...
	light.Diffuse.g = 10.f;
	light.Diffuse.b = 10.f;
	light.Diffuse.a = 0.f;
...
void CLevel_Event::Tick(_float fTimeDelta)
{
	__super::Tick(fTimeDelta);
	CSoundMgr::Get_Instance()->PlaySound(L"BGM16_mp3.mp3", CSoundMgr::BGM1, 0.3f);
	...
	_gameins->Collison_Sheare(LEVEL_EVENT, L"Monster", L"Layer_Player");
	_gameins->Collison_Knockback(LEVEL_EVENT, L"Layer_HitBox", L"Monster");
	_gameins->Collison_Knockback(LEVEL_EVENT, L"Layer_PlayerHitBox", L"Layer_Player");
	_gameins->SuperCollison_Rect(LEVEL_EVENT, L"Layer_Player", BoxsPos);
	_gameins->SuperCollison_Rect(LEVEL_EVENT, L"Monster", BoxsPos);
	_gameins->Collison_Knockback(LEVEL_EVENT, L"Layer_HitBox", L"Obelisk");
...
HRESULT CLevel_Event::Render()
{
	if (FAILED(__super::Render()))
		return E_FAIL;

	SetWindowText(g_hWnd, TEXT("Event."));

	return S_OK;
}

HRESULT CLevel_Event::Ready_Layer_Camera(const _tchar * pLayerTag)
{
...
HRESULT CLevel_Event::RandomSpawn()
{
	CGameInstance*		pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
	_float3 SpawnPos;
	while (true)
	{
		...
		CTransform*		pTargetTransform = (CTransform*)pGameInstance->Get_Component(LEVEL_EVENT, TEXT("Layer_Player"), TEXT("Com_Transform"));
...
HRESULT CLevel_Event::Ready_Layer_Player(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_Event::Ready_Layer_BackGround(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
	_uint num = 48;
...
HRESULT CLevel_Event::Ready_Layer_Effect(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_Event::Ready_Layer_UI(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
CLevel_Event * CLevel_Event::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CLevel_Event*	pInstance = new CLevel_Event(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct()))
	{
		MSG_BOX(TEXT("Failed to Created CLevel_Event"));
