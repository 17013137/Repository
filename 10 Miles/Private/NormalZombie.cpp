...
_int CNormalZombie::Tick(_float fTimeDelta)
{
	if (true == m_bDead)
		return 1;
	...
	if (m_pStatusCom->Status_Tick(fTimeDelta) == CStatus::STATUS_STATE_HIT)
	{
		...
		CSoundMgr::Get_Instance()->PlaySound(L"Monster_Beaten2.wav", CSoundMgr::MONSTER1_BEATEN, 0.6f);
...
HRESULT CNormalZombie::Render()
{
	...
	for (int i = 0; i < 9; ++i)
	{
		...
		if (FAILED(m_vTT[i]->Bind_OnGraphicDevice(i + iTexture * 9)))
			return E_FAIL;
...
HRESULT CNormalZombie::SetUp_Components()
{
	...
	for (int i = 0; i < 10; ++i)
	{
		...
		if (FAILED(__super::SetUp_Components(szPath4[i], m_Current_Level, TEXT("Prototype_Component_Texture_NormalZombie"), (CComponent**)&m_CubeTT)))
			return E_FAIL;
...
void CNormalZombie::SetUp_OnTerrain()
{
...
void CNormalZombie::Falling(_float fTimeDelta)
{
...
*/
void CNormalZombie::Down(_float fTimeDelta)
{
	_float3 RH = _float3(-0.05f, 0.03f, 0.03f);
	_float3 LH = _float3(0.02f, 0.05, 0.05f);
	_float3 RF = _float3(-0.01f, 0.03f, 0.02f);
...
void CNormalZombie::Up(_float fTimeDelta)
{
	_float3 RH = _float3(-0.05f, 0.03f, 0.03f);
	_float3 LH = _float3(0.02f, 0.05, 0.05f);
	_float3 RF = _float3(-0.01f, 0.03f, 0.02f);
...
CNormalZombie * CNormalZombie::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	...
	{
		MSG_BOX(TEXT("Failed to Created CNormalZombie"));
