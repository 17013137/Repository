...
HRESULT CBlood_Effect::NativeConstruct_Prototype()
{
...
HRESULT CBlood_Effect::NativeConstruct(void * pArg)
{
	...
	m_pTransformCom->LookAt(m_Effect_Type.Target + _float3(rand()%20-10.f, rand() % 10 - 5.f, rand() % 20 - 10.f));
	m_pTransformCom->Go_Straight(1.8f);
	m_pTransformCom->Turn(m_pTransformCom->Get_State(CTransform::STATE_RIGHT), D3DXToRadian(90.0f));
	m_pTransformCom->Turn(_float3(0.f, 1.f, 0.f), D3DXToRadian(45.f));
	m_pTransformCom->Go_Straight(-0.5f);
	return S_OK;
}

_int CBlood_Effect::Tick(_float fTimeDelta)
{
...
HRESULT CBlood_Effect::SetUp_Components()
{
	/* For.Com_Transform */
	...
	/* For.Com_Texture */
	if (FAILED(__super::SetUp_Components(TEXT("Com_Texture"), m_Current_Level, TEXT("Prototype_Component_Texture_Blood"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
