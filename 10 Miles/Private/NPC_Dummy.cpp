#include "stdafx.h"
#include "..\Public\NPC_Dummy.h"
#include "GameInstance.h"
#include "SoundMgr.h"
#include "KeyMgr.h"
#include "EventMgr.h"

CNPC_Dummy::CNPC_Dummy(LPDIRECT3DDEVICE9 pGraphic_Device)
...
HRESULT CNPC_Dummy::NativeConstruct_Prototype()
{
...
HRESULT CNPC_Dummy::NativeConstruct(void * pArg)
{
...
_int CNPC_Dummy::Tick(_float fTimeDelta)
{
...
void CNPC_Dummy::LateTick(_float fTimeDelta)
{
...
HRESULT CNPC_Dummy::SetUp_Components()
{
	/* For.Com_Transform */
...
HRESULT CNPC_Dummy::SetUp_OnTerrain()
{
	CGameInstance*			pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
void CNPC_Dummy::Set_Part(int Part, int Local, _float3 Pos)
{
	_float4x4	MX;
...
void CNPC_Dummy::Set_Part2(CSubTransform* Part, _float3 Pos)
{
	_float4x4	MX;
	D3DXMatrixIdentity(&MX);
...
void CNPC_Dummy::Run(_float fTimeDelta)
{
	//움직이는 속도
...
void CNPC_Dummy::Walk(_float fTimeDelta)
{
	//움직이는 속도
...
void CNPC_Dummy::Falling(_float fTimeDelta)
{
...
void CNPC_Dummy::Down(_float fTimeDelta)
{
	_float3 RH = _float3(-0.05f, 0.03f, 0.03f);
	_float3 LH = _float3(0.02f, 0.05, 0.05f);
	_float3 RF = _float3(-0.01f, 0.03f, 0.02f);
...
void CNPC_Dummy::Up(_float fTimeDelta)
{
	_float3 RH = _float3(-0.05f, 0.03f, 0.03f);
	_float3 LH = _float3(0.02f, 0.05, 0.05f);
	_float3 RF = _float3(-0.01f, 0.03f, 0.02f);
...
void CNPC_Dummy::NO(_float fTimeDelta)
{
...
void CNPC_Dummy::YES(_float fTimeDelta)
{
	_float3 RH = _float3(0.f, 0.6f, 0.6f)*fTimeDelta;
...
void CNPC_Dummy::FOLDED(_float fTimeDelta)
{
...
void CNPC_Dummy::HI(_float fTimeDelta)
{
...
void CNPC_Dummy::Stay(_float fTimeDelta)
{
...
int CNPC_Dummy::ActionMoving(_float fTimeDelta)
{
	if (CEventMgr::Get_Instance()->Get_OnAir() == false)
		return 0;

	if (CEventMgr::Get_Instance()->Get_EventCnt() == 0 && CEventMgr::Get_Instance()->Get_isInitSet(1) == true && flag == false) {
		NO(fTimeDelta);
		return 1;
	}

	if (CEventMgr::Get_Instance()->Get_EventCnt() == 4 && CEventMgr::Get_Instance()->Get_isInitSet(0) == true && flag == false) {
		HI(fTimeDelta);
		return 1;
	}

	if (CEventMgr::Get_Instance()->Get_EventCnt() == 5 && CEventMgr::Get_Instance()->Get_isInitSet(0) == true && flag == false) {
		HI(fTimeDelta);
		return 1;
	}
...
void CNPC_Dummy::Attack(_float fTimeDelta)
{
...
CNPC_Dummy * CNPC_Dummy::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CNPC_Dummy*	pInstance = new CNPC_Dummy(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CNPC_Dummy"));
