#include "stdafx.h"
#include "..\Public\ZombieSpitter.h"
...
_int CZombieSpitter::Tick(_float fTimeDelta)
{
	if (true == m_bDead)
		return 1;
	...
	if (LockOn == true || State == STATE_ATTACK)
	{
		if (State != STATE_FALLING && State != STATE_DOWN && State != STATE_UP && delay < 0)
		{
			Action = true;
			MoveNo = false;
			Attack(fTimeDelta);
	...
	else if (D3DXVec3Length(&vLook) > 1.f && D3DXVec3Length(&vLook) < 7.f && Action == false)
	{
		LockOn = true;
		Action = true;
		MoveNo = false;
	...
	else if (TargetAngle < 30.f && D3DXVec3Length(&vLook) < 13.f && Action == false)
	{
		LockOn = true;
		Action = true;
		MoveNo = false;
	...
	else if (D3DXVec3Length(&vLook) > 5.f && D3DXVec3Length(&vLook) < 15.f && Action == false)
	{
		Action = true;
		MoveNo = false;
		find(fTimeDelta);
	...
	if (m_pStatusCom->Status_Tick(fTimeDelta) == CStatus::STATUS_STATE_HIT)
	{
		...
		CSoundMgr::Get_Instance()->StopSound(CSoundMgr::MONSTER3_BEATEN);
		CSoundMgr::Get_Instance()->PlaySound(L"Monster_Beaten3.wav", CSoundMgr::MONSTER3_BEATEN, 0.6f);
...
HRESULT CZombieSpitter::Render()
{
	...
	for (int i = 4; i < 6; ++i)
	{
		...
		if (FAILED(m_vTT[8]->Bind_OnGraphicDevice(8)))
			return E_FAIL;
...
HRESULT CZombieSpitter::SetUp_Components()
{
	...
	for (int i = 0; i < 10; ++i)
	{
		...
		if (FAILED(__super::SetUp_Components(szPath4[i], m_Current_Level, TEXT("Prototype_Component_Texture_ZombieSpitter"), (CComponent**)&m_CubeTT)))
			return E_FAIL;
...
void CZombieSpitter::Attack(_float fTimeDelta)
{
	...
	else if (AnFrame >= 48 && AnFrame < 64)
	{
		...
		m_Effect_Type.Pos = m_pTransformCom->Get_State(CTransform::STATE_POSITION)+ StatusmDesc.fLook;
...
void CZombieSpitter::Falling(_float fTimeDelta)
{
...
*/
void CZombieSpitter::Down(_float fTimeDelta)
{
	_float3 RH = _float3(-0.05f, 0.03f, 0.03f);
	_float3 LH = _float3(0.02f, 0.05, 0.05f);
	_float3 RF = _float3(-0.01f, 0.03f, 0.02f);
...
void CZombieSpitter::Up(_float fTimeDelta)
{
	_float3 RH = _float3(-0.05f, 0.03f, 0.03f);
	_float3 LH = _float3(0.02f, 0.05, 0.05f);
	_float3 RF = _float3(-0.01f, 0.03f, 0.02f);
...
void CZombieSpitter::find(_float fTimeDelta)
{
...
void CZombieSpitter::Crawl(_float fTimeDelta)
{
	//움직이는 속도
...
void CZombieSpitter::Walk(_float fTimeDelta)
{
	//움직이는 속도
...
void CZombieSpitter::Move(_float fTimeDelta)
{
	//움직이는 속도
	_float3 RH = _float3(-0.0015f, 0.01f, 0.01f);
	_float3 LH = _float3(0.0015f, 0.01f, 0.01f);
...
void CZombieSpitter::Stay(_float fTimeDelta)
{
