...
CGrandfa_Dummy::CGrandfa_Dummy(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
}

CGrandfa_Dummy::CGrandfa_Dummy(const CGrandfa_Dummy & rhs)
	: CGameObject(rhs)

{
}

HRESULT CGrandfa_Dummy::NativeConstruct_Prototype()
{
...
HRESULT CGrandfa_Dummy::NativeConstruct(void * pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, _float3(39.f, 0.6f, 35.f));
	m_col_Range = 1.f;
	m_col_Size = { 1.f, 1.f };
...
_int CGrandfa_Dummy::Tick(_float fTimeDelta)
{
	...
	//m_pTransformCom->Set_State(CTransform::STATE_LOOK, TargetPos);
...
void CGrandfa_Dummy::LateTick(_float fTimeDelta)
{
...
HRESULT CGrandfa_Dummy::SetUp_Components()
{
	/* For.Com_Transform */
...
HRESULT CGrandfa_Dummy::SetUp_OnTerrain()
{
	CGameInstance*			pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
...
void CGrandfa_Dummy::Set_Part(int Part, int Local, _float3 Pos)
{
	_float4x4	MX;
...
void CGrandfa_Dummy::Set_Part2(CSubTransform* Part, _float3 Pos)
{
	_float4x4	MX;
	D3DXMatrixIdentity(&MX);
...
void CGrandfa_Dummy::Run(_float fTimeDelta)
{
	//움직이는 속도
...
void CGrandfa_Dummy::Walk(_float fTimeDelta)
{
	//움직이는 속도
...
void CGrandfa_Dummy::Falling(_float fTimeDelta)
{
...
void CGrandfa_Dummy::Down(_float fTimeDelta)
{
	_float3 RH = _float3(-0.05f, 0.03f, 0.03f);
	_float3 LH = _float3(0.02f, 0.05, 0.05f);
	_float3 RF = _float3(-0.01f, 0.03f, 0.02f);
...
void CGrandfa_Dummy::Up(_float fTimeDelta)
{
	_float3 RH = _float3(-0.05f, 0.03f, 0.03f);
	_float3 LH = _float3(0.02f, 0.05, 0.05f);
	_float3 RF = _float3(-0.01f, 0.03f, 0.02f);
...
void CGrandfa_Dummy::NO(_float fTimeDelta)
{
	_float3 RH = _float3(0.f, 0.6f, 0.6f)*fTimeDelta;
	_float3 LH = _float3(0.5f, 0.f, 0.f)*fTimeDelta;
	_float3 RF = _float3(-0.01f, 0.03f, 0.02f)*fTimeDelta;
	_float3 LF = _float3(0.01f, 0.05f, 0.04f)*fTimeDelta;
	...
	{
		//	//머리
		m_vTF[0]->Turn(_float3(1.f, 0.f, 0.f), fTimeDelta);
		m_vTF[2]->Set_State(CSubTransform::STATE_POSITION, m_vTF[2]->Get_State(CSubTransform::STATE_POSITION) + RH);

	}

	else if (AnFrame < 0.75)
	{
		////머리
		m_vTF[0]->Turn(_float3(0.f, 1.f, 0.f), fTimeDelta * 2);
		m_vTF[2]->Set_State(CSubTransform::STATE_POSITION, m_vTF[2]->Get_State(CSubTransform::STATE_POSITION) + LH);
	}
	else if (AnFrame < 1.25)
	{
...
void CGrandfa_Dummy::YES(_float fTimeDelta)
{
	_float3 RH = _float3(0.f, 0.6f, 0.6f)*fTimeDelta;
	_float3 LH = _float3(0.f, 0.6f, 0.6f)*fTimeDelta;
	_float3 RF = _float3(-0.01f, 0.03f, 0.02f)*fTimeDelta;
	_float3 LF = _float3(0.01f, 0.05f, 0.04f)*fTimeDelta;
	_float4x4 Rot, Far, Loc, End;
	if (State != STATE_YES)
...
void CGrandfa_Dummy::FOLDED(_float fTimeDelta)
{
	_float3 RH = _float3(-1.f, 0.4f, 0.6f)*fTimeDelta;
	_float3 LH = _float3(1.f, 0.4f, 0.6f)*fTimeDelta;
	_float3 RF = _float3(-0.01f, 0.03f, 0.02f)*fTimeDelta;
	_float3 LF = _float3(0.01f, 0.05f, 0.04f)*fTimeDelta;
	_float4x4 Rot, Far, Loc, End;
	if (State != STATE_FOLDED)
	...
	else if (AnFrame < 4.25)
	{
		////머리
		m_vTF[0]->Turn(_float3(1.f, 0.f, 0.f), -fTimeDelta);

	}
	else if (AnFrame < 5.25)
	{
		////머리
		m_vTF[2]->Set_State(CSubTransform::STATE_POSITION, m_vTF[2]->Get_State(CSubTransform::STATE_POSITION) - RH);
		m_vTF[3]->Set_State(CSubTransform::STATE_POSITION, m_vTF[3]->Get_State(CSubTransform::STATE_POSITION) - LH);
...
void CGrandfa_Dummy::HI(_float fTimeDelta)
{
	_float3 RH = _float3(0.f, 2.f, 0.6f)*fTimeDelta;
	_float3 LH = _float3(0.7f, 0.f, 0.f)*fTimeDelta;
	_float3 RF = _float3(-0.01f, 0.03f, 0.02f)*fTimeDelta;
	_float3 LF = _float3(0.01f, 0.05f, 0.04f)*fTimeDelta;
	_float4x4 Rot, Far, Loc, End;
	if (State != STATE_HI)
	...
	{
		////머리
		//m_vTF[0]->Turn(_float3(0.f, 0.f, 1.f), fTimeDelta / 5);
		m_vTF[2]->Set_State(CSubTransform::STATE_POSITION, m_vTF[2]->Get_State(CSubTransform::STATE_POSITION) + LH);
	}
	else if (AnFrame < 1.25)
	{
	...
	{
		////머리
	//	m_vTF[0]->Turn(_float3(1.f, 0.f, 0.f), -fTimeDelta / 5);
		m_vTF[2]->Set_State(CSubTransform::STATE_POSITION, m_vTF[2]->Get_State(CSubTransform::STATE_POSITION) - LH);
...
void CGrandfa_Dummy::Stay(_float fTimeDelta)
{
...
int CGrandfa_Dummy::ActionMoving(_float fTimeDelta)
{
	if (CEventMgr::Get_Instance()->Get_OnAir() == false)
		return 0;

	if (CEventMgr::Get_Instance()->Get_EventCnt() == 1 && CEventMgr::Get_Instance()->Get_isInitSet(3) == true && flag == false) {
		YES(fTimeDelta);
		return 1;
	}

	if (CEventMgr::Get_Instance()->Get_EventCnt() == 4 && CEventMgr::Get_Instance()->Get_isInitSet(1) == true && flag == false) {
		YES(fTimeDelta);
		return 1;
	}

	//if (CEventMgr::Get_Instance()->Get_EventCnt() == 5 && CEventMgr::Get_Instance()->Get_isInitSet(2) == true && flag == false) {
	//	YES(fTimeDelta);
	//	return 1;
	//}
...
void CGrandfa_Dummy::Attack(_float fTimeDelta)
{
...
CGrandfa_Dummy * CGrandfa_Dummy::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CGrandfa_Dummy*	pInstance = new CGrandfa_Dummy(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CGrandfa_Dummy"));
...
CGameObject * CGrandfa_Dummy::Clone(void * pArg)
{
	CGrandfa_Dummy*	pInstance = new CGrandfa_Dummy(*this);
